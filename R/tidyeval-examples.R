...
summarize_var_new <- function(data,variable){
		summarise(data,
			...
			pct1_25 = quantile({{variable}}, p = .25, na.rm = TRUE),
			pct1_75 = quantile({{variable}}, p = .75, na.rm = TRUE)
		)
}
...
summarize_var_new(nlsy, age_bir, sex_cat)
summarize_var_new(nlsy, nsibs, sex_cat)
...
summarize_two_vars <- function(data,variable1,variable2){
	summarise(data,
						cor = cor({{variable1}},{{variable2}}, use = "pairwise.complete.obs"),
						cov = cov({{variable1}},{{variable2}}, use = "pairwise.complete.obs"),
						)
}

summarize_two_vars(nlsy$income, nlsy$age_bir)
